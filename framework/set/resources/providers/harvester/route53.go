...
// CreateRoute53Record is a function that will set the Harvester Route 53 record configuration in the main.tf file.
func CreateRoute53Record(rootBody *hclwrite.Body, terraformConfig *config.TerraformConfig) {
	...
	zoneBlockBody.SetAttributeValue(name, cty.StringVal(terraformConfig.HarvesterConfig.HarvesterRoute53Zone))
	zoneBlockBody.SetAttributeValue(privateZone, cty.BoolVal(false))
}

// CreateRoute53InternalRecord is a function that will set the Harvester Route 53 record configuration in the main.tf file.
